function [train_X0, train_Y0,test_X0, test_Y0] = Preprocessing_sonar

%Ecoli

parentPath = fileparts(mfilename('fullpath'));

realData = readtable([parentPath,'\datasets\sonar.data'], 'Filetype', 'text', 'ReadVariableNames', false);
attributes = realData{:,1:60};
real_labels = realData{:,61};

labels=zeros(size(real_labels,1),1);
for i=1:size(labels,1)
    if strcmp(real_labels(i,1),'M')
        labels(i,1)=1;
    else
        labels(i,1)=-1;
    end
